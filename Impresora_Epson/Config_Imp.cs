             ...
             , int iRecLineChars, string strDec, string StrSerieFactura, string StrSerieBoleta, string StrConceptoVisaUnica, string _codigo_hash)
         {
               ...
               try
               {
                   ...
                   if (dt_venta != null)
                   {
                       if (dt_venta.Rows.Count > 0)
                       {
                           ...
                           string _nota = dt_venta.Rows[0]["nota"].ToString();
                           ...
                           PrintReceiptFooter(Double.Parse(dSubtotal.ToString()), _igv,
                               ...
                               _tipodoc, _percepcionm, _percepcionp, _estadook, _codigo_hash, _nota, _monto_nc, _referencia_nc);
           ...
           private static void PrintReceiptFooter(double subTotal, decimal varIGV, double tax, double discount, string footerText, string resolucion, string resolucion1, string resolucion2, string varDocumentTypeID, double percepcion, decimal porcpercepcion, string estadotk, string _codigo_hash,string _nota,Decimal monto_nc,string referencia_nc)
           {
               ...
               if (estadotk == "2")
               {
                   ...
                   //EN ESTE CASO VAMOS A PONER EL PAGO CON NOTA DE CREDITO
                   ...
                   Decimal _var_sb = Convert.ToDecimal(((subTotal - discount) + mtoigv));
                   ...
                   decimal _total_nc = _var_sb - monto_nc;

                   decimal _total_pagar_nc = (_var_sb - monto_nc) + Convert.ToDecimal(percepcion);


                   //PrintTextLine(SetNormal + offSetString + new string('-', (_recLineChars / 3)));
                   PrintTextLine_ZoneSubTot(SetNormal + espaciof + "DESC NC  (-) " + "" + "   " + 0.ToString(_strDec, _myCIintl).Replace("0.00", "") + (monto_nc).ToString("#0.00").PadLeft(9));


                   PrintTextLine(SetNormal + offSetString + new string('-', (_recLineChars / 3)));
                   //PrintTextLine_ZoneSubTot(SetNormal + offSetString + "TOTAL           " + 0.ToString(_strDec, _myCIintl).Replace("0.00", "") + ((subTotal - discount) + mtoigv).ToString("#0.00").PadLeft(9));
                   PrintTextLine_ZoneSubTot(SetNormal + offSetString + "TOTAL           " + 0.ToString(_strDec, _myCIintl).Replace("0.00", "") + (_total_nc).ToString("#0.00").PadLeft(9));
                   ...
                   if (_total_nc!=0)
                   {
                       ...
                       PrintTextLine(SetNormal + offSetString + new string('-', (_recLineChars / 3)));
                       PrintTextLine_ZoneSubTot(SetNormal + offSetString + "TOTAL A PAGAR   " + 0.ToString(_strDec, _myCIintl).Replace("0.00", "") + (_total_pagar_nc).ToString("#0.00").PadLeft(9));
                   }

                   PrintTextLine(SetNormal + new string('-', _recLineChars));
                   PrintTextLineCompleta(referencia_nc);
                   //PrintTextLine(SetNormal + new string('-', _recLineChars));      
              ...
              string strTipo, string StrSerieFactura, string StrSerieBoleta, string StrConceptoVisaUnica,string strautorizacion)
           {
               ...
               if (strDocumentTypeID == "2")
               {
                   ...
                   PrintTextLine(SetNormal + SetBold + String.Format("TICKET: {0}", strInvoiceNumber));
               }
