...
namespace www.aquarella.com.pe.Aquarella.Logistica
{
    public partial class ordersForm : System.Web.UI.Page
    {
        ...
        public static Boolean getArticlePremio(Coordinator cust,Users user)
        {
            try
            {               
                List<Articles_Sizes> artSizes = new List<Articles_Sizes>();
                ...
                DataSet dsArt = Article.getArticle(cust._ppremio.Replace("-", string.Empty).Trim(), 17);

                if (dsArt == null || dsArt.Tables[0].Rows.Count == 0)
                    throw new Exception("El artículo digitado es inexistente.");

                newLineOrder = Order_Dtl.getNewLineOrderPremio(dsArt.Tables[0]);
                artSizes = Articles_Sizes.getObjectSizes(dsArt.Tables[1], false);
                ...
                newLineOrder._premio = "S";
                newLineOrder._premId = cust._pPremID;
                newLineOrder._size = cust._pTalla;
                newLineOrder._qty = cust._pCantidad;
                newLineOrder._commissionPctg = 0m;
                newLineOrder._commissionDesc = (0).ToString(_currency);
                newLineOrder._commission = 0;
                newLineOrder._dscto = 0;

                //newLineOrder._price = 0m;
                newLineOrder._lineTotal = 0m;
                newLineOrder._lineTotDesc = (0).ToString(_currency);
                newLineOrder._priceDesc = (0).ToString(_currency);
                newLineOrder._priceigvDesc = (0).ToString(_currency);
                
                List<Order_Dtl> orderLines = new List<Order_Dtl> ();
                
                orderLines.Add(newLineOrder);

                //fin obtenemos la talla y las demas cosas

                Boolean valor =doLiquidationPremio(cust,user, "", 0m, orderLines, 0, false);
       ...
       public static Boolean doLiquidationPremio(Coordinator cust, Users user,  string typeLiq, Decimal _varPercepcion, List<Order_Dtl> order, int pagospos = 0, Boolean _pago_credito = false)
        {
            // Generar liquidación.
            Boolean valor = true;
            string[] noOrder;
            string strTipoPago = "008";
            decimal comision_customer;
            decimal mtopercepcion;
            decimal total = 0;
            decimal subTotal = Math.Round(order.Sum(x => x._lineTotal), 2, MidpointRounding.AwayFromZero);
            ...
            if (order.Count > 0)
            {
                ...
                foreach (Order_Dtl fila in order)
                {
                    item_temp += 1;
                    Order_Dtl_Temp item = new Order_Dtl_Temp();
                    item.items = item_temp;
           ...
           comision_customer = cust._commission;
            ...
            mtopercepcion = 0m; //cust._mtopercepcion;
            ...
            noOrder = Liquidations_Hdr.Gua_Mod_Liquidacion(user._bas_id, cust._idCust, string.Empty, comision_customer, 0, string.Empty, string.Empty, order, mtopercepcion, 1, "", "", 0, 0, "", "", total, dtpago, _pago_credito, cust._percepcion, orderLines_Temp, strTipoPago);
